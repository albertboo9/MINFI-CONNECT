...
import {
    ...
    Users, ClipboardCheck, BookOpen, Target, Database,
...
const NAV_MAP = {
    ...
    [ROLES.MANAGER]: [
        ...
        { icon: Target, labelKey: 'nav.trainingSurvey', path: '/trainings' },
    ],
    [ROLES.HRM]: [
        { icon: LayoutDashboard, labelKey: 'common.dashboard', path: '/' },
    ...
    [ROLES.TECH]: [
        ...
        { icon: Award, labelKey: 'nav.certValidation', path: '/certifications' },
    ...
    [ROLES.DIRECTOR]: [
        ...
        { icon: BookMarked, labelKey: 'nav.exportReports', path: '/certifications' },
    ],
};
...
export default function AppShell({ children }) {
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-themed">
            ...
            >
                {/* Logo */}
                <div className="h-[60px] flex items-center px-3 gap-3 flex-shrink-0 border-b" style={{ borderColor: 'var(--border)' }}>
                    ...
                    <AnimatePresence>
                        {sidebarOpen && (
                            <motion.div initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -8 }} className="overflow-hidden">
                                <p className="font-heading font-black text-sm leading-none" style={{ color: 'var(--text-primary)' }}>TPGS <span style={{ color: accent }}>·</span> MINFI</p>
                                <p className="text-[9px] font-bold tracking-[0.2em] uppercase mt-0.5" style={{ color: 'var(--text-muted)' }}>Cameroun · 2026</p>
                            </motion.div>
                        )}
                    </AnimatePresence>
                </div>
                ...
                {/* User chip */}
                <div className="px-2 py-3 border-b flex-shrink-0" style={{ borderColor: 'var(--border)' }}>
                    <div className={clsx('flex items-center gap-2.5 px-2 py-1.5 rounded-xl', !sidebarOpen && 'justify-center')}>
                        ...
                        {sidebarOpen && (
                            <div className="overflow-hidden min-w-0">
                                <p className="text-xs font-semibold truncate leading-tight" style={{ color: 'var(--text-primary)' }}>{currentUser.name}</p>
                                <p className="text-[9px] font-bold tracking-[0.15em] uppercase truncate mt-0.5" style={{ color: accent }}>{t(`roles.${activeRole}`)}</p>
                            </div>
                        )}
                    </div>
                </div>
                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            <button key={item.path} onClick={() => navigate(item.path)}
                                className={clsx('nav-item w-full text-left', isActive && 'active', !sidebarOpen && 'justify-center px-0')}
                                style={isActive ? { color: accent, backgroundColor: accent + '14' } : {}}
                                title={!sidebarOpen ? t(item.labelKey) : undefined}
                            >
                                <item.icon size={16} className="flex-shrink-0" />
                                {sidebarOpen && <span className="truncate flex-1">{t(item.labelKey)}</span>}
                                {sidebarOpen && isActive && <div className="w-1.5 h-1.5 rounded-full ml-auto" style={{ backgroundColor: accent }} />}
                            </button>
                        );
                    })}
                </nav>
                ...
                <div className="p-2 border-t flex-shrink-0" style={{ borderColor: 'var(--border)' }}>
                    {sidebarOpen && <div className="mb-2"><RoleSwitcher /></div>}
                    ...
                    <div className={clsx('flex gap-1', !sidebarOpen && 'flex-col')}>
                        <button onClick={toggleTheme} className="nav-item flex-1 justify-center" title={isDark ? t('common.lightMode') : t('common.darkMode')}>
                            ...
                            {sidebarOpen && <span className="text-xs truncate">{isDark ? t('common.lightMode') : t('common.darkMode')}</span>}
                        </button>
                        <button onClick={toggleLang} className="nav-item flex-1 justify-center" title="Switch language">
                            <Globe size={14} />
                            {sidebarOpen && <span className="text-xs font-mono">{lang === 'fr' ? 'EN' : 'FR'}</span>}
                        </button>
                    </div>

                    <button onClick={toggleSidebar} className="nav-item w-full justify-center mt-1" title="Toggle sidebar">
                        <ChevronLeft size={14} className={clsx('transition-transform duration-300', !sidebarOpen && 'rotate-180')} />
                        {sidebarOpen && <span className="text-xs">{t('common.collapse', 'Réduire')}</span>}
                    </button>
                </div>
            </motion.aside>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                {/* Header */}
                <header className="h-[60px] flex items-center justify-between px-6 border-b flex-shrink-0"
                    style={{ backgroundColor: 'var(--bg-secondary)', borderColor: 'var(--border)' }}>
                    <div>
                        <p className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>
                            {currentNavItem ? t(currentNavItem.labelKey) : t('common.dashboard')}
                        </p>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={toggleNotifPanel}
                            ...
                            style={{ backgroundColor: notifPanelOpen ? 'var(--bg-hover)' : '' }}>
                            <Bell size={16} style={{ color: 'var(--text-muted)' }} />
                            {unread > 0 && (
                                <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full animate-pulse-ring" style={{ backgroundColor: '#EF4444', border: '2px solid var(--bg-secondary)' }} />
                            )}
                        </button>
                        ...
                        <div className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-xl border"
                            ...
                            <div className="w-2 h-2 rounded-full" style={{ backgroundColor: accent, animation: 'pulse 2s infinite' }} />
                            <span className="text-[10px] font-semibold" style={{ color: 'var(--text-muted)' }}>{currentUser.grade}</span>
                        </div>
                    </div>
                </header>
