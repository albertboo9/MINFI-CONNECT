...
function AddTrainingModal({ open, onClose, onAdd }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (!form.title || !form.provider) {
      toast.error("Titre et prestataire obligatoires");
      return;
    }
    onAdd(form);
    playSuccess();
    toast.success("Nouvelle formation référencée", {
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title="Référencer une formation"
      subtitle="Gestion du catalogue de sourcing"
      size="sm"
      footer={
        <>
          ...
          <button type="submit" form="add-cat-form" className="btn-primary">
      ...
      <form id="add-cat-form" onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            placeholder="Ex: Analyse de données financières"
          />
        </div>
        ...
        <div className="flex items-center gap-6 p-3 bg-themed-hover rounded-xl border border-dashed border-themed">
          ...
          <select
            className="bg-transparent text-xs font-bold text-themed outline-none border-none"
            value={form.category}
            onChange={(e) =>
...
function RequestTrainingModal({
  ...
  const handleSubmit = (e) => {
    ...
    const targetName =
      targetType === "self"
        ? "vous-même"
        : teamMembers.find((m) => m.id === selectedAgentId)?.name;
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <button type="submit" form="request-cat-form" className="btn-primary">
    ...
    >
      <form id="request-cat-form" onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="p-4 rounded-xl bg-tpgs-emerald/5 border border-tpgs-emerald/20 flex gap-4">
          ...
          <div>
            <p className="text-xs font-black text-themed truncate">
              {course.title}
            </p>
            <p className="text-[10px] text-themed-muted font-bold uppercase tracking-wider">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="En quoi cette formation est-elle pertinente pour le poste ?"
          />
        </div>

        <div className="flex items-center gap-2 p-3 rounded-xl bg-themed-hover text-[10px] text-themed-muted italic">
          <AlertCircle size={14} className="text-blue-400" />
          Une notification sera envoyée à l'agent concerné et à la hiérarchie
        ...
        <>
          ...
          {course.isFree && isOp ? (
            <button
              ...
              className="btn-approve animate-pulse"
            >
      ...
      <div className="space-y-8">
        <div className="flex flex-wrap gap-2">
          ...
          <span className="badge badge-slate">
            {course.certification || "Attestation MINFI"}
          </span>
          ...
          {course.isFree && (
            <span className="badge badge-emerald">Gratuité totale</span>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="card p-4 flex items-center gap-3 border-themed bg-themed-hover/20">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-themed capitalize">
                {course.modality || "Hybride"}
              </p>
            </div>
          </div>
          <div className="card p-4 flex items-center gap-3 border-tpgs-emerald/20 bg-tpgs-emerald/5">
            ...
            <div>
              ...
              <p className="text-sm font-black text-tpgs-emerald">
                {course.cost > 0
                  ? `${course.cost.toLocaleString()} FCFA`
                  : "Sourcing Interne (0 F)"}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div>
            <h3 className="font-heading font-black text-lg text-themed mb-2">
              Description Opérationnelle
            </h3>
            <p className="text-sm text-themed-muted leading-relaxed">
              {course.description ||
                "Formation spécialisée conçue pour renforcer les capacités opérationnelles des agents du Ministère des Finances dans un environnement dématérialisé."}
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
            <div className="space-y-4">
              <p className="section-label flex items-center gap-2 font-black">
                <Target size={14} className="text-tpgs-emerald" /> Objectifs de
                Performance
              </p>
              <ul className="space-y-3">
                {[
                  ...
                  <li
                    key={i}
                    className="text-xs text-themed flex items-start gap-3"
                  >
                    <CheckCircle2
                      size={14}
                      className="text-tpgs-emerald flex-shrink-0 mt-0.5"
                    />{" "}
                    <span>{i}</span>
                  </li>
                ))}
            ...
            <div className="space-y-4">
              <p className="section-label flex items-center gap-2 font-black">
                <Users size={14} className="text-blue-400" /> Profils éligibles
              </p>
              <p className="text-xs text-themed-muted leading-relaxed bg-themed-hover p-4 rounded-xl border border-themed">
                Cette session s'adresse prioritairement aux agents de la
                Catégorie A et B en charge du pilotage financier ou de l'audit
                interne au sein du MINFI.
              </p>
            </div>
          </div>
        </div>
...
export default function Catalogue() {
  ...
  const handleAddCourse = (data) => {
    const newC = {
      ...
      id: `c${Date.now()}`,
      modality: "blended",
      cost: data.isFree ? 0 : 75000,
      certification: "Certificat TMS",
    };
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-20">
      <CourseModal
        ...
        onShowRequest={handleQuickRequest}
      ...
      <section className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-themed">
        <div>
          <h1 className="font-heading font-black text-4xl text-themed tracking-tight">
      ...
      <section className="flex flex-col lg:flex-row gap-4">
        <div className="relative flex-1 group">
          <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
            <Search
              size={20}
              className="text-themed-muted group-focus-within:text-tpgs-emerald transition-colors"
            />
          </div>
          <input
            value={search}
          ...
          {["all", "job", "transversal", "specific", "executive"].map((f) => (
            <button
              ...
              className={clsx(
                ...
                catFilter === f
                  ? "bg-tpgs-emerald text-white border-tpgs-emerald"
                  : "bg-themed-card text-themed-muted border-themed hover:border-themed-muted hover:bg-themed-hover",
              )}
            >
              ...
              >
                <div className="p-7 flex-1 space-y-5">
                  <div className="flex justify-between items-start">
                    <span
                      className={clsx(
                        "badge px-3 py-1 text-[9px] font-black",
                        theme.badge,
                      )}
                    >
                      {theme.label}
                    ...
                    <div className="flex gap-1.5 leading-none">
                      {c.isFree && (
                        <span className="bg-tpgs-emerald/20 text-tpgs-emerald text-[9px] font-black py-1 px-2 rounded-lg">
                          OFFERT
                        </span>
                      )}
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          toast.info("Ajouté à vos favoris");
                        }}
                        className="p-1.5 hover:bg-themed-hover rounded-lg text-themed-muted hover:text-amber-400 transition-colors"
                      >
                        <Bookmark size={14} />
                      </button>
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-themed-muted font-bold flex items-center gap-1.5">
                      <span className="w-1 h-1 rounded-full bg-themed-muted" />{" "}
                      {c.provider}
                    </p>
                  </div>
                  ...
                  <p className="text-xs text-themed-muted line-clamp-3 leading-relaxed min-h-[50px] italic">
                    {c.description ||
                      "Description pédagogique en cours de rédaction par les services de la Direction des Ressources Humaines."}
                  </p>

                  <div className="flex items-center gap-4 text-[10px] font-black text-themed-muted uppercase font-mono tracking-tighter">
                    <span className="flex items-center gap-1.5 p-1.5 bg-themed-hover rounded-lg">
                ...
                <div className="p-5 bg-themed-hover/40 border-t border-themed/50 flex items-center justify-between mt-auto group-hover:bg-themed-hover/60 transition-colors">
                  <div className="flex flex-col">
                    <p className="text-[10px] font-bold text-themed-muted uppercase tracking-widest">
                      {c.cost > 0 ? "Budget Estimé" : "Sourcing"}
                    </p>
                    <p className="text-base font-black text-tpgs-emerald">
                      {c.cost > 0
                        ? `${(c.cost / 1000).toFixed(0)}k FCFA`
                        : "GRATUIT"}
                    </p>
                  </div>
                  <div className="flex gap-2">
      ...
      {filtered.length === 0 && (
        <div className="card p-32 text-center animate-fade-in border-dashed border-4 border-themed">
          ...
          <h3 className="font-heading font-black text-2xl text-themed mb-2">
            Aucun titre disponible
          </h3>
          <p className="text-sm text-themed-muted max-w-sm mx-auto">
            Nous n'avons trouvé aucune formation correspondant à "{search}".
            Essayez des termes comme "Audit", "Fiscalité" ou "Gestion".
          </p>
        </div>
      )}
    </div>
